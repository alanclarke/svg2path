var fs = require("fs"),
    ...
    eve = elemental.eve,
    raphael_ = require('raphael-browserify'),
...
function rappar(svg) {
    ...
    parser(svg);
    parser.end();
    return allToPaths(items);
}


function allToPaths(items) {
    var paths = [items.length];
    items.forEach(function(item, i){
        var path = cleanPath(toPath(item));
        paths[i] = path;
    });
    return paths.join(' ');
}
...
function toPath(item) {
    ...
    if (item.transform) {
        ...
        new_path = raphael_.transformPath(p, item.transform)
        p = new_path.toString();
    }
